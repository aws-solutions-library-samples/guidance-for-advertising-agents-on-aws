...
1. **Schema-First Authority:** The `.json` files are the source of truth. Do not modify agent behavior by editing
   ...
   - **In this repo, `agentcore.json` is itself generated** and gitignored. Edit
     `agentcore/agentcore.fragment.json` (or `agents/_shared/agentcore.base.json` for anything common to
     all agents) and re-run `python3 render_agentcore_json.py` from the adcp root. An edit made directly
     to `agentcore.json` is reverted by the next render.
...
├── agentcore/
│   ├── agentcore.json      # Main project config (AgentCoreProjectSpec). Generated + gitignored
