...
# ── Valid modes ─────────────────────────────────────────────────────
# a2a = Strands + A2A runtime (a2a_main.py, requirements-a2a.txt). The others
# are the legacy CrewAI HTTP/MCP paths.
...
# =============================================================================
# A2A Runtime deployment (Strands)  — serves the Strands seller over A2A
# =============================================================================
deploy_a2a_runtime() {
  local agent_name="${1:-${HTTP_AGENT_NAME}}"
  echo ""
  echo "============================================="
  echo "  Deploying A2A Runtime (Strands): ${agent_name}"
  echo "============================================="
  ...
  local configure_args=(
    ...
    -n "${agent_name}"
    -rf infra/aws/agentcore/requirements-a2a.txt
    -p A2A
    -r "${REGION}"
    --non-interactive
    --deployment-type container
  ...
  # Strands runtime env — no CrewAI vars, no ROUTING_MODE. The internal
  # FastAPI+MCP server the seller agent reads from starts in-process.
  local env_args=(
    # src-layout import fix: the package is `ad_seller` under /app/src. Baked in
    # here so `--mode a2a` works standalone (not only via deploy_aamp_agents.sh).
...
if [[ "${TEST_ONLY}" == "false" ]]; then
  ...
  case "${DEPLOY_MODE}" in
    ...
    chat)
      ...
      ;;
    a2a)
      deploy_a2a_runtime "${HTTP_AGENT_NAME}"
      ;;
  esac
