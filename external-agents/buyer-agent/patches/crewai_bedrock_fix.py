...
# --- AAMP local patch: never hand Bedrock an assistant-final conversation ---
# Claude on the Bedrock Converse API rejects a request whose message list ends
# with an assistant turn ("This model does not support assistant message
# prefill. The conversation must end with a user message."). _sanitize_tool_blocks
# can leave an assistant message last after dropping orphaned tool blocks;
# re-assert a trailing user turn here (baked in from the ecosystem deploy patch).
