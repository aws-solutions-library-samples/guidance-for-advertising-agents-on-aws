...
# Validate deploy mode. http = legacy CrewAI runtime; a2a = Strands + A2A runtime.
# (MCP deferred — Issue 19.)
if [[ "${DEPLOY_MODE}" != "http" && "${DEPLOY_MODE}" != "a2a" ]]; then
  echo "ERROR: Only --mode http or --mode a2a is supported for buyer agent"
  exit 1
fi
...
if [[ "${TEST_ONLY}" == "false" ]]; then
  ...
  # Configure — entrypoint/protocol/requirements depend on mode
  if [[ "${DEPLOY_MODE}" == "a2a" ]]; then
    _ENTRY="src/ad_buyer/interfaces/agentcore/a2a_main.py"
    _PROTO="A2A"
    _REQ="infra/aws/agentcore/requirements-a2a.txt"
  else
    _ENTRY="src/ad_buyer/interfaces/agentcore/http_main.py"
    _PROTO="HTTP"
    _REQ="infra/aws/agentcore/requirements.txt"
  fi
  echo ""
  echo ">>> Configuring agent (mode=${DEPLOY_MODE}, protocol=${_PROTO})..."
  _configure_args=(
    -e "${_ENTRY}"
    -n "${AGENT_NAME}"
    -rf "${_REQ}"
  ...
  agentcore configure "${_configure_args[@]}"

  # Deploy
  echo ""
  echo ">>> Deploying to AgentCore (mode=${DEPLOY_MODE})..."
  if [[ "${DEPLOY_MODE}" == "a2a" ]]; then
    # Strands + A2A runtime. Seller-wiring + model/port env are exported by the
    # parent deploy (scripts/deploy_aamp_agents.sh); forward whatever is present.
    _deploy_env=(
      # src-layout import fix: package `ad_buyer` under /app/src.
      --env "PYTHONPATH=/app/src:/app"
      --env "DEFAULT_LLM_MODEL=${DEFAULT_LLM_MODEL:-global.anthropic.claude-sonnet-5}"
      --env "AWS_REGION=${REGION}"
      --env "AWS_DEFAULT_REGION=${REGION}"
      --env "AGENTCORE_A2A_PORT=${AGENTCORE_A2A_PORT:-9000}"
      --env "ANTHROPIC_API_KEY=not-used-with-bedrock"
      --env "STORAGE_TYPE=sqlite"
      --env "DATABASE_URL=sqlite:///:memory:"
    )
    [[ -n "${BUYER_COORDINATOR_MODEL:-}" ]] && _deploy_env+=(--env "BUYER_COORDINATOR_MODEL=${BUYER_COORDINATOR_MODEL}")
