#!/bin/bash
# ============================================================================
# sync_config.sh — Push local AAMP-resolved config to DynamoDB + S3 and validate
#
# Runs four steps:
#   1. Upload global_configuration.json to DynamoDB (via upload_agent_configs_to_dynamodb.py)
#   2. Sync global_configuration.json to the S3 data + UI buckets
#   3. Invalidate CloudFront cache
#   4. Validate all three sources are in sync (via validate_config_consistency.py)
#
# Usage:
#   ./scripts/sync_config.sh --stack-prefix a4a --unique-id omixaj --region us-east-1
#   ./scripts/sync_config.sh --stack-prefix a4a --unique-id omixaj --region us-east-1 --profile agnts4ad
#   ./scripts/sync_config.sh --stack-prefix a4a --unique-id omixaj --region us-east-1 --skip-dynamo
#   ./scripts/sync_config.sh --stack-prefix a4a --unique-id omixaj --region us-east-1 --validate-only
# ============================================================================
...
# Derived names (matches cloudformation/infrastructure-core.yml bucket naming
# and scripts/upload_agent_configs_to_dynamodb.py's expected AgentConfigTable name)
...
if [ ! -f "$CONFIG_FILE" ]; then
    # Auto-resolve from template if the live config doesn't exist yet
...
# ── Pre-flight: block sync if the local file has unresolved AAMP placeholders ──
if [ "$VALIDATE_ONLY" = false ]; then
    PLACEHOLDERS=$(grep -oE '\$\{[^}]+\}' "$CONFIG_FILE" 2>/dev/null | sort -u)
    if [ -n "$PLACEHOLDERS" ]; then
        ...
        echo -e "  Resolve placeholders first with scripts/resolve_config.py, using values"
        echo -e "  from:"
