...
│    ├─ invoke_specialist(AAMPBuyerCrewAgent)                  │
│    │    → invoke_agent_runtime (buyer ARN)                   │
│    └─ invoke_specialist(AAMPSellerCrewAgent)                 │
...
```
invoke_specialist(agent_name="AAMPSellerCrewAgent", agent_prompt="Show available CTV inventory")
```
