...
- [For IT Admins — Deployment](#for-it-admins--deployment)
  ...
  - [Manual deployment (no orchestrator)](#manual-deployment-no-orchestrator)
  - [Retrieving the client secret](#retrieving-the-client-secret)
  - [Callback URLs on the app client](#callback-urls-on-the-app-client)
...
- **Client ID**
- **Client Secret**
- **Token URL** (e.g., `https://a4a-xxxxx.auth.<REGION>.amazoncognito.com/oauth2/token`)
- **Authorization URL** (e.g., `https://a4a-xxxxx.auth.<REGION>.amazoncognito.com/oauth2/authorize`)
...
python agentcore/deployment/deploy_a4a_mcp_handler.py \
  --stack-prefix a4a --unique-id <ID> --region <REGION> --profile <PROFILE> \
  --iam-target
```
...
scripts/deploy-ecosystem.sh --skip-quick-gateway ...
```
...
If you would rather not run `deploy-ecosystem.sh`, these are the equivalent steps. You need
**four** inputs:

| Input | Where to find it |
|---|---|
| `<PREFIX>` | the `--stack-prefix` used at deploy time |
| `<UID>` | the unique id, recorded in `.unique-id-<PREFIX>-<REGION>` at the repo root |
| `<REGION>` | the `--region` used at deploy time |
| `<PROFILE>` | your own AWS profile. Omit if credentials come from the environment |

```bash
# 1. Confirm the Lambda exists (created in Phase 2)
...
aws lambda update-function-configuration \
  --function-name <PREFIX>-a4a-mcp-handler-<UID> \
  --environment "Variables={AGENT_CONFIG_TABLE=<PREFIX>-AgentConfig-<UID>,GUIDANCE_RUNTIME_ARN=<RUNTIME_ARN>}" \
  --region <REGION> --profile <PROFILE>

# 4. Create the gateway and register the target
python agentcore/deployment/deploy_a4a_mcp_handler.py --mode oauth-cognito \
  --stack-prefix <PREFIX> --unique-id <UID> --region <REGION> --profile <PROFILE>
```

The script prints the Gateway URL, Client ID, Token URL, Authorization URL and Scope, and
writes them to `.quick-gw-<PREFIX>-<UID>.json`.

#### Retrieving the client secret

The client secret is **not printed and not written to that file**. It goes to SSM as a
`SecureString`, so a captured deploy log never contains a live credential:

```bash
aws ssm get-parameter \
  --name /<PREFIX>/quick-gateway/<UID>/client-secret \
  --with-decryption --query Parameter.Value --output text \
  --region <REGION> --profile <PROFILE>
```

#### Callback URLs on the app client

The app client is created with two callback URLs. Both are intentional:

| URL | Why |
|---|---|
| `https://us-east-1.quicksight.aws.amazon.com/sn/oauthcallback` | The Quick Suite OAuth callback. It is `us-east-1` for every customer regardless of which region your stack is in, because that is where the Quick Suite endpoint lives. Do not change it to match your region |
| `http://localhost:3000/callback` | For testing the OAuth flow locally without Quick Suite. Plain HTTP is acceptable here only because the client also requires a secret, so the redirect on its own grants nothing. Remove it if you do not want a localhost redirect registered |

#### Teardown

Cleanup removes the gateway, its target, the gateway IAM role, the SSM parameter, and the
Cognito app client and resource server. It deliberately **does not** delete the Cognito user
pool domain: the Angular UI's SSO sign-in reads the same domain, so removing it here would
break federated login. The domain goes with the user pool when the CloudFormation stack is
deleted.
