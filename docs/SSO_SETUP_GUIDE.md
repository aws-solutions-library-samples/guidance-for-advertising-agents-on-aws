...
## Step 1: Ensure Your Cognito Domain Exists

SSO needs a Cognito hosted-UI domain on the user pool. **The standard deployment does not
create one.** Check first:
...
Two ways it may already be there:

- You created it previously, by hand.
- You deployed the **optional Quick MCP Gateway (Phase 12)**, which creates a domain named
  `<STACK_PREFIX>-<UNIQUE_ID>` on the same user pool as a side effect. See
  [QUICK_SETUP_GUIDE.md](QUICK_SETUP_GUIDE.md).

If the command returns `None` or empty, create one. The prefix must be globally unique across
all AWS accounts in the region:

```bash
aws cognito-idp create-user-pool-domain \
  --user-pool-id <USER_POOL_ID> \
  --domain <STACK_PREFIX>-<UNIQUE_ID> \
  --region <REGION>
```

> Both SSO and the Quick gateway read this one domain, so do not delete it while either is in
> use. Quick gateway teardown deliberately leaves it in place for this reason.
...
Either through the deployment script:
...
or by calling the generator directly:
...
If the user pool has no hosted-UI domain, no `sso` section is written and the generator says so.
That is deliberate: an `sso` block without a working domain would render a sign-in button that
cannot complete.
